...
class AgentEager(Agent):
    def place_stone(self, board, color):
        '''
        Like AgentFollow, plays "around" the opponent, but tries to first win itself by playing in the same row, col, diag or anti-diag.
        ...
        if color == 1:
            if open_xs_black[2][0][0] != -1:
                start_pos_1, end_pos_1, start_pos_2, end_pos_2 = open_xs_black[2][0]
                return np.array([start_pos_1, end_pos_1])
            if half_open_xs_black[2][0][0] != -1:
                pos = half_open_xs_black[2][0]
                return chose_empty_from_half_open(board, pos)
        else:
            if open_xs_white[2][0][0] != -1:
                start_pos_1, end_pos_1, start_pos_2, end_pos_2 = open_xs_black[2][0]
                return np.array([start_pos_1, end_pos_1])
            if half_open_xs_white[2][0][0] != -1:
                pos = half_open_xs_black[2][0]
                return chose_empty_from_half_open(board, pos)
